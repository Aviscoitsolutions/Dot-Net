...
{
    internal class MySqlDataAccess : ITelemetryAccess
    {
        private MySqlConnection connection;
        private MySqlTransaction transaction;
        private long Imei;

        public MySqlDataAccess(long? imei)
        {
            if (imei != null)
                this.Imei = (long)imei;
            this.connection = new MySqlConnection(Settings.Default.DatabaseConnectionString);
        }
        ...
        {
            using (MySqlCommand command = new MySqlCommand("INSERT INTO device_errors (imei, time, server_time, function, function_warning) VALUES (@imei, @time, @server_time, @function, @function_warning)", connection))
            {
        ...
        public void TelemetryInsert(MergedData item)
        {
            ...
            if (item.Lls != null)
            {
                ...
                queryGenerator.AddParam("lls_temp_add0", item.Lls.Value.temp_add[0]);
                queryGenerator.AddParam("lls_temp_add1", item.Lls.Value.temp_add[1]);
                queryGenerator.AddParam("lls_temp_add2", item.Lls.Value.temp_add[2]);
                queryGenerator.AddParam("lls_temp_add3", item.Lls.Value.temp_add[3]);
            }
        ...
        {
            using (MySqlCommand command = new MySqlCommand("INSERT INTO outputs (output_id, output_data, status, expiration_date) VALUES (@output_id, @output_data, @status, @expiration_date); select last_insert_id();", connection))
            {
        ...
        {
            using (MySqlCommand command = new MySqlCommand("UPDATE outputs SET status = @status WHERE id = @id;", connection))
            {
        ...
        public List<int> CheckConfirmationExpirations()
        {
            ...
            using (MySqlCommand command = new MySqlCommand("SELECT * FROM outputs WHERE status = 0 OR status = 3 OR status = 6;", connection, transaction))
            {
