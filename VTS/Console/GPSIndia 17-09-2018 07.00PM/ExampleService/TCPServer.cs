        ...
        private void ReadCallback(IAsyncResult ar)
        {
            ...
            if (state.BytesRecieved > 0 && localBuffer != null)
            {
                ...
                if (myArray.Length == 15)
                {
                    ...
                    Console.WriteLine("Ignetion:" + nowignation);

                    telemetry l = db.telemetries.OrderByDescending(u => u.id).FirstOrDefault(u => u.imei == Convert.ToInt64(Helper.imei));
                    l.digital_inputs = Convert.ToInt32(nowignation);
                    db.SubmitChanges();
