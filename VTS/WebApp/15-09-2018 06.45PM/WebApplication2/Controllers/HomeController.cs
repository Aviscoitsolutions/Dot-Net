...
namespace WebApplication2.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        //Save logo of dealer
        public JsonResult Savelogos(string id, string profile)
        {
            Tbl_DealerRegistration t1 = db.Tbl_DealerRegistrations.Single(t => t.Dealer_Id == Convert.ToInt32(id));
            t1.Logo = profile.Trim();
            db.SubmitChanges();
            string response = "Image Saved Sucessfully";
        ...
        public JsonResult getmainlogo(product product)
        {
            var username = (string)Session["username"];
            ...
            try
            {
                var uId = (from a in db.Tbl_Users where a.username == username.ToString() select a.UserID).SingleOrDefault();
                UserId = Convert.ToInt32(uId);
                var dealerid = (from a in db.Tbl_DeviceRegistrations where a.UserId == UserId select a.Dealer_ID).ToList();
                var logo = (from a in db.Tbl_DealerRegistrations where a.Dealer_Id == dealerid[0]select  a.Logo ).SingleOrDefault();
